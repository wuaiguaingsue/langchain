...
文档加载器将数据加载为标准LangChain文档格式。

每个文档加载器都有其特定的参数，但它们都可以通过相同的方式使用.load方法调用。
示例用法如下：
...
参见此指南作为起点：[如何：加载网页](/docs/how_to/document_loader_web)。
...
参见此指南作为起点：[如何：加载PDF文件](/docs/how_to/document_loader_pdf)。
